...
async def index(request):
    ...
    if index is not None:
        index=int(index)
        message=smtp_server.messages[index] if index <len(smtp_server.messages) else None
        
    text=template('index',{'messages':smtp_server.messages,'message':message,'dumps':dumps,'index':index})
    return web.Response(
        text=text,
